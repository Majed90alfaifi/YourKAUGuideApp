...
public class PlansButtonOptionsHandler {

    private final ArrayList<String> PlansOptions;
    Context context;
    public int optionIndex = 0;

    private final MainActivity mainActivity;

    public PlansButtonOptionsHandler(ArrayList<String> buttonOptions, Context context, MainActivity mainActivity) {
        this.PlansOptions = buttonOptions;
    ...
    public void showButtonOptions(ViewGroup linearLayout, MessageRVAdapter.BotViewHolder holder) {
        ...
        for (int i = optionIndex; i < Math.min(optionIndex + 3, PlansOptions.size()); i++) {
            Button button = createButton(PlansOptions.get(i), holder);
